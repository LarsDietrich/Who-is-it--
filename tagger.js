/*!
 * Face.com Tagging Widget v1.1.0 (alpha) 
 * http://face.com/
 *
 * Copyright 2010, 
...
function Face_Tagger(_FaceClientAPI)
{
	...
	function load(oImg, options)
	{
		...
		{	
			var refCallback = (options.success == undefined || typeof options.success != "function")? undefined : options.success;
			var refTagClick = options.tagClick;
			
			var bReadOnly		= (typeof options.readonly != "boolean")? false : options.readonly;
			var bManualTags		= (typeof options.click_add_tag != "boolean")? false : options.click_add_tag;
			var bAddTagButton	= (typeof options.add_tag_button != "boolean")? false : options.add_tag_button;
			var bNamesAsLinks	= (typeof options.names_as_links != "boolean")? false : options.names_as_links;
			var bResizable		= (typeof options.resizable != "boolean")? false : options.resizable;
			var bShowTagsList	= (typeof options.tags_list != "boolean")? false : options.tags_list;
			var bFadeTags		= (typeof options.fade != "boolean")? false : options.fade;
			var bTagsVisible	= (typeof options.tags_visible != "boolean")? true : options.tags_visible;
			var bShowLoading	= (typeof options.show_loading != "boolean")? true : options.show_loading;
		...
		try
		{
			...
			{	
                ...
                var options = { 'uids':'friends@facebook.com',
...
//		namespace train owners_ids callback_url
	...
	function drawTag(oWrapper, sTagId, sFullName, oImage, iConfirmed, iManual, _iX, _iY, _iWidth, _iHeight, refTagClick, bReadOnly, bNamesAsLinks, bNewTag, bResizable, bFadeTags)
	{
		...
		if (design == 'zeedevil')
		{
			dbug("log", "*6*6*6* ZEE DEVIL *6*6*6*");
