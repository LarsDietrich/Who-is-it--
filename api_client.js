/*!
 * Face.com Rest API JavaScript Library v1.0.1 (alpha) 
 * http://face.com/
 *
 * Copyright 2010, 
...
function Face_ClientAPI(_apiKey)
{
	...
	function faces_detect(urls, callback)
	{
		var method = "faces/detect";
		validateInput(urls, 'url', 'url', method);
		validateInput(callback, 'function', 'callback', method);

		makeRequest(method, { urls: urls }, callback);
	...
	function faces_recognize(urls, options, callback)
	{
		...
		if (!empty(options.train))			params.namespace = options.train;
